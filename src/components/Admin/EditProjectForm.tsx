...
const EditProjectForm = ({ projectDetails }: any) => {
  ...
  const handleDeleteImage = (tagIndex: number, imageIndex: number) => {
    ...
    if (updatedTags?.[tagIndex]?.images?.length > 1) {
      updatedTags[tagIndex].images.splice(imageIndex, 1);
      setFormData({
        ...formData,
        tags: updatedTags,
      });
    } else {
      toast.error("Need to have at least one image");
    }
  };
  ...
  const handleDeleteTag = (tagIndex: number) => {
    ...
    if (updatedTags?.length > 1) {
      ...
      toast.error("Need to have at least one tag");
    }
  };
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-gray-100">
      ...
      >
        ...
        <div>
          {formData.tags.map((tag: any, tagIndex: number) => (
            <div
              key={tagIndex}
            ...
            >
              ...
              {tag.images.map((image: any, imageIndex: number) => (
                <div
                  key={imageIndex}
